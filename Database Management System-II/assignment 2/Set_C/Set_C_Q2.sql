...
-- Set C - Question 2
-- Function to find the teacher teaching maximum number
-- of subjects
-- ============================================================

-- Q2. Write a function to find the teacher who is teaching
...
INSERT INTO student_teacher VALUES (103, 1, 'Physics');
...
INSERT INTO student_teacher VALUES (105, 2, 'Computer');
...
DECLARE
    ...
    subj_count INTEGER;
BEGIN
    SELECT TRIM(t.t_name), COUNT(DISTINCT TRIM(st.subject))
    INTO tname, subj_count
    FROM teacher t
    JOIN student_teacher st ON t.tno = st.tno
    GROUP BY t.tno, t.t_name
    ORDER BY COUNT(DISTINCT TRIM(st.subject)) DESC
    LIMIT 1;

    IF tname IS NULL THEN
        RAISE NOTICE 'No records found.';
    ELSE
        RAISE NOTICE 'Teacher teaching maximum subjects: % (% subjects)', tname, subj_count;
    END IF;
END;
$$ LANGUAGE plpgsql;
