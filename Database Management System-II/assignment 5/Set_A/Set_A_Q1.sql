...
-- Set A - Question 1
-- ============================================================
-- Q1. Write a trigger that is executed whenever an actor is
--     deleted from the Actor table. Display an appropriate message.
-- ============================================================
...
-- ========================
DROP TRIGGER IF EXISTS trg_actor_delete ON actor;
DROP FUNCTION IF EXISTS fn_actor_delete();

CREATE OR REPLACE FUNCTION fn_actor_delete()
RETURNS TRIGGER AS $$
BEGIN
    RAISE NOTICE 'Actor "%" with role "%" has been deleted from the Actor table.', OLD.a_name, OLD.role;
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_actor_delete
    ...
    FOR EACH ROW
    EXECUTE FUNCTION fn_actor_delete();

-- ========================
-- Test DML
-- ========================
-- This DELETE will fire the trigger and display a notice message
DELETE FROM actor WHERE a_name = 'Tom Hardy';
